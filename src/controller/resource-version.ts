...
export class ResourceVersionController {
    ...
    async create(ctx) {
        ...
        const latestVersion = isEmpty(resourceVersions) ? null : resourceVersions[0].version;
        if (!isEmpty(latestVersion) && !semver.gt(version, latestVersion)) {
            throw new ArgumentError(ctx.gettext('resource-version-must-be-greater-than-latest-version', latestVersion));
        }

...
export class ResourceService implements IResourceService {
    ...
    async _buildDependencyTree(dependencies, maxDeep = 100, currDeep = 1, omitFields = []) {
        ...
        const results = [], tasks = [];
        for (let i = 0, j = resourceList.length; i < j; i++) {
            let {resourceId, resourceName, resourceType, baseUpcastResources} = resourceList[i];
            let {versionId, versionRange, versions, version} = dependencyMap.get(resourceId) as any;
            let versionInfo = versionInfoMap.get(versionId);
            let result: any = {

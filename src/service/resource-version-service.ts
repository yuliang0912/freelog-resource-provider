...
export class ResourceVersionService implements IResourceVersionService {
    ...
    async createResourceVersion(resourceInfo: ResourceInfo, options: CreateResourceVersionOptions): Promise<ResourceVersionInfo> {
        ...
        if (resourceVersion.status === 1) {
            ...
            let modifyModel = {$addToSet: {resourceVersions: versionInfo}};
            if (isFirstVersion) {
                modifyModel['baseUpcastResources'] = resourceVersion.upcastResources;
            }
            await this.resourceProvider.updateOne({_id: resourceInfo.resourceId}, modifyModel);
        }
    ...
    async updateResourceVersion(versionInfo: ResourceVersionInfo, options: UpdateResourceVersionOptions): Promise<ResourceVersionInfo> {
        ...
        for (let i = 0, j = options.resolveResources.length; i < j; i++) {
            let {resourceId, contracts} = options.resolveResources[i] as any;
            let intrinsicResolve = updatedResolveResources.find(x => x.resourceId === resourceId);
        ...
        updatedResolveResources.forEach(resolveResource => resolveResource.contracts.forEach(item => {
            let key = `${resolveResource.resourceId}_${item.policyId}`;
            let signedContractInfo = contractMap.get(key);
    ...
    async _validateDependencies(resourceId, dependencies): Promise<object[]> {
        ...
        const dependResourceMap = await this.resourceProvider.find({_id: {$in: dependencies.map(x => x.resourceId)}})
            .then(list => new Map(list.map(x => [x.resourceId, x])));

        const invalidDependencies = [], invalidResourceVersionRanges = [];
        dependencies.forEach(dependency => {
            let resourceInfo = dependResourceMap.get(dependency.resourceId);
    ...
    async _cycleDependCheck(resourceId, dependencies, deep = 1): Promise<{ ret: boolean, deep?: number }> {
        ...
        const dependResourceInfos = await this.resourceProvider.find({_id: {$in: dependencies.map(x => x.resourceId)}}, 'resourceVersions');
    ...
    async _getRealUpcastAndResolveResources(dependencies, baseUpcastResources): Promise<{ upcastResources: object[], backlogResources: object[], allUntreatedResources: object[] }> {
        ...
        let upcastResources = [], backlogResources = [], allUntreatedResources = [];
        if (!dependencies.length) {
            return {upcastResources, backlogResources, allUntreatedResources}
        }

        const dependResources = await this.resourceProvider.find({_id: {$in: dependencies.map(x => x.resourceId)}});
